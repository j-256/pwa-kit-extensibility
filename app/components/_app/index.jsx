...
const App = (props) => {
    ...
    const { data: privacyPolicy } = useQuery({
        ...
        queryFn: () => {
            return fetch(`${getAppOrigin()}/mobify/proxy/ocapi/s/RefArch/dw/shop/v20_2/content/privacy-policy?client_id=6d17e164-4331-482a-908e-ff8d007f7782`)
                .then(res => res.json())
                .then((json) => {
                    console.log(json)
                    return json
    ...
    useEffect(() => {
        ...
        const fetchStore = async () => {
            const res = await fetch(
                `${getAppOrigin()}/mobify/proxy/ocapi/s/RefArch/dw/shop/v20_2/stores?latitude=${GEO_LOCATION.lat}&longitude=${GEO_LOCATION.long}&client_id=6d17e164-4331-482a-908e-ff8d007f7782`
            )
            if (res.ok) {
                    ...
                    <CurrencyProvider currency={currency}>
                        ...
                        <Box id="app" display="flex" flexDirection="column" flex={1}>
                            ...
                            {closestStore && (
                                ...
                                >
                                    <InfoOutlineIcon />
                                    <Text fontWeight="bold" pl={1}>Closest Store:
                                    </Text>
                                    <Text pl={2}>
                                        {closestStore.name} -
                                        {closestStore.address1},{' '}
                                        {closestStore.state_code},
                                        {closestStore.postal_code}
                                    </Text>
                                </Box>
                            )}
